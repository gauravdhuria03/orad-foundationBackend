...
/**
 *  Change Status of user from backend
 * @access private
 * @return json
 * Created by Gaurav
 * 
 * Created Date 21-06-2021
 */
 function changeStatusFromBackend(req, res) {
    let userId = req.body.userId;
    console.log("userId====", userId);
    async function changeStatusFromBackend() {
        try {
            let cond = {
                _id: userId
            }
            ...
            if (!finalResult.status) {
                ...
                finalResult.data.isActive = req.body.isActive;
                return res.json(Response(constant.statusCode.ok, constant.statusCode.profileUpdated, finalResult.data));
            }
        } catch (error) {
            console.log(error);
    ...
    changeStatusFromBackend().then(function (data) {
    });
}
 ...
 function usersList(req, res) {
    async function usersList() {
        try {
            ...
            let resultObject = await query.findDataBySortSkipLimit(User, condition, sortObject, count, skip);
             console.log("resultObject==",resultObject);                      
             let totalCount = await query.countData(User, condition);
